<div class="p-6 bg-neutral-0 rounded-4 border border-neutral-40 mb-10">
    <div class="d-flex align-items-center justify-content-between flex-wrap gap-3">
        <h5 class="clr-primary-300 d-inline-block mb-0"> Apartment </h5>
        <ul class="list list-row gap-3 align-items-center">
            <li>
                <span class="d-inline-block clr-neutral-600"> Share </span>
            </li>
            <li>
                <a href="#" class="link w-8 h-8 d-grid place-content-center bg-primary-50 clr-primary-300 rounded-circle :bg-primary-300 :clr-neutral-0">
                    <i class="fab fa-facebook-f"></i>
                </a>
            </li>
            <li>
                <a href="#" class="link w-8 h-8 d-grid place-content-center bg-primary-50 clr-primary-300 rounded-circle :bg-primary-300 :clr-neutral-0">
                    <i class="fab fa-twitter"></i>
                </a>
            </li>
            <li>
                <a href="#" class="link w-8 h-8 d-grid place-content-center bg-primary-50 clr-primary-300 rounded-circle :bg-primary-300 :clr-neutral-0">
                    <i class="fab fa-instagram"></i>
                </a>
            </li>
            <li>
                <a href="#" class="link w-8 h-8 d-grid place-content-center bg-primary-50 clr-primary-300 rounded-circle :bg-primary-300 :clr-neutral-0">
                    <i class="fab fa-linkedin-in"></i>
                </a>
            </li>
        </ul>
    </div>
    <h2 class="mt-4 mb-8">
        About 50% Discount For Cash or Installment With 10% Down payment in Fifth Square
    </h2>
    <ul class="list list-row flex-wrap align-items-center list-divider-dot gap-4 gap-md-0">
        <li>
            <div class="d-flex align-items-center gap-2">
                <span class="material-symbols-outlined mat-icon clr-secondary-400"> distance </span>
                <p class="mb-0"> New Cairo </p>
            </div>
        </li>
        <li>
            <p class="mb-0"> Status : <span class="clr-primary-300">Completed</span>
            </p>
        </li>

        <li>
            <p class="mb-0">
                <span class="clr-neutral-500">Developed by :</span>Tatweer Misr
            </p>
        </li>
    </ul>


    <div class="hr-dashed my-8"></div>
    <ul class="list list-row flex-wrap align-items-center list-divider-dot gap-4 gap-md-0">
        <li>
            <div class="d-flex align-items-center gap-2">
                <span class="material-symbols-outlined mat-icon"> home_work </span>
                <p class="mb-0"> 7 Room </p>
            </div>
        </li>

        <li>
            <div class="d-flex align-items-center gap-2">
                <span class="material-symbols-outlined mat-icon"> bathtub </span>
                <p class="mb-0"> 3 Bath </p>
            </div>
        </li>
        <li>
            <div class="d-flex align-items-center gap-2">
                <span class="material-symbols-outlined mat-icon"> drag_pan </span>
                <p class="mb-0"> 702 sft </p>
            </div>
        </li>

    </ul>
</div>


